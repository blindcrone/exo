...
class TrainingNode(Node):
  ...
  def on_node_status(self, request_id, opaque_status):
    if DEBUG >= 8: print(opaque_status)
    try:
  ...
  async def broadcast_supported_engines(self, supported_engines_names: List[str]):
    await self.broadcast_opaque_status("", json.dumps({"type": "supported_inference_engines", "node_id": self.id, "engines": supported_engines_names})
)
  ...
  async def process_prompt(self, base_shard: Shard, prompt: str, image_str: Optional[str] = None, request_id: Optional[str] = None, inference_state: Optional[str] = None) -> Optional[np.ndarray]:
    ...
    status_base = await self.node_status() | {
    ...
    asyncio.create_task(
      self.broadcast_opaque_status(
        request_id,
        json.dumps(status_base | {"status": "start_process_prompt"})
      )
    )
  ...
  async def process_batch(self, base_shard: Shard, batch: List[str], image_str: Optional[List[str]] = None) -> Optional[np.ndarray]:
#    token_batch = [tokenizer.encode(i) for i in batch]
...
#    batch_arr = np.zeros((batch_size, max(lengths)), np.int32)
    ...
    for prompt in batch:
      ...
      callback = self.on_token.register(callback_id)
      if self.topology_viz:
        self.topology_viz.update_prompt(request_id, prompt)
      try:
        print(f"Processing prompt: {prompt}")
        await self.process_prompt(base_shard, prompt, None, request_id=request_id)
  ...
  async def process_tensor(
    ...
    shard = self.get_current_shard(base_shard)
    base_status = await self.node_status() | { 
      "base_shard": base_shard.to_dict(),
      "shard": shard.to_dict(),
      "request_id": request_id,
    }
