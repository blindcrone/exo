...
class StandardNode(Node):
  ...
  async def _train_example(
    ...
    if shard.is_last_layer():
      ...
      backgrad = await self.forward_example(shard, step, target, length, True, request_id, self.get_partition_index(offset = 1))
  ...
  async def _eval_example(
    ...
    if shard.is_last_layer():
      ...
      step = await self.inference_engine.infer_tensor(request_id, shard, example)
      loss = await self.forward_example(shard, step, target, length, False, request_id, self.get_partition_index(offset = 1))
    return loss        
  ...
  async def _process_example(
    ...
    try:
      ...
      if train:
        result = await self._train_example(shard, example, target, length, request_id)
      else:
        result = await self._eval_example(shard, example, target, length, request_id)
    except Exception as e:
