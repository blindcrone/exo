...
class TestCoreMemoryWeightedPartitioningStrategy(unittest.TestCase):
  def test_partition(self):
    ...
    topology.update_node(
      "node1",
      DeviceCapabilities(model="test1", chip="test1", memory=3000, flops=DeviceFlops(fp32=0, fp16=0, int8=0), cores=4096),
    )
    topology.update_node(
      "node2",
      DeviceCapabilities(model="test2", chip="test2", memory=1000, flops=DeviceFlops(fp32=0, fp16=0, int8=0), cores=128),
    )
    topology.update_node(
      "node3",
      DeviceCapabilities(model="test3", chip="test3", memory=6000, flops=DeviceFlops(fp32=0, fp16=0, int8=0), cores=8),
    )
    ...
    strategy = CoreMemoryWeightedPartitioningStrategy()
    ...
    self.assertEqual(
      partitions,
      [
        Partition("node1", 0.0, 0.3),
        Partition("node2", 0.3, 0.4),
